#
# This script takes the following steps to obtain a report from gosec.
#
...
write-verbose 'Step 3: Running gosec...'

gosec -version

write-output '##tool = GoSec'   | out-file -LiteralPath $outputPath -Append
gosec @($options) -fmt json -no-fail $scanRequestConfig.packages | out-file -LiteralPath $outputPath -Append
if ($LASTEXITCODE -ne 0) {
	Exit-Script "Unexpected exit code from gosec ($LASTEXITCODE)."
}
write-verbose 'Done'
